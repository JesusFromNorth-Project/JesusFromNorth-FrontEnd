...
async function buscarPacientePorDni(dni) {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}?dni=${encodeURIComponent(dni)}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function eliminarPaciente(idPaciente) {
    try {
        ...
        const confirmacion = confirm('¿Está seguro de que desea eliminar este paciente? Esta acción no se puede deshacer.');
        if (!confirmacion) {
            return false;
        }
...
async function buscarPacientesPorApellido(apellido, pagina = 0) {
    try {
        ...
        const response = await fetch(
            `${API_BASE_URL}list/?lastName=${encodeURIComponent(apellido)}&page=${pagina}`, 
            {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.message || 'Error al buscar pacientes por apellido');
        }
