 ...
 * @param {Object} paciente - Datos del paciente a editar
 */
...
function validarFormularioPacientes(formData) {
  ...
  } else if (!/^[0-9]{8}$/.test(formData.get("dni").trim())) {
  ...
  if (!formData.get("edad")?.trim()) {
    errors.push("La edad es obligatoria");
  } else if (
    isNaN(parseInt(formData.get("edad"))) ||
    parseInt(formData.get("edad")) <= 0
  ) {
  ...
  if (!formData.get("telefono")?.trim() && !formData.get("celular")?.trim()) {
    errors.push("Debe proporcionar al menos un teléfono o celular");
  }
...
async function obtenerPacientePorId(idPaciente) {
  try {
    ...
    if (!response.ok) {
      ...
      const error = await response.json().catch(() => ({}));
      throw new Error(
        error.message || "Error al obtener los datos del paciente"
      );
    }
...
function actualizarTablaPacientes(pacientes) {
  ...
  if (!tbody) {
    console.error(
      "No se encontró el elemento tbody para la tabla de pacientes"
    );
    return;
  }
...
async function cargarPacientes(pagina = 0) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || "Error al obtener la lista de pacientes"
      );
    }
